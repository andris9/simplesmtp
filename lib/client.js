...
function SMTPClient(port, host, options){
    ...
    if(!this.options.name){
        ...
        defaultHostname = (oslib.hostname && oslib.hostname()) ||
 ...
 */
SMTPClient.prototype._onError = function(err, type){
    if(type && type != "Error"){
        err.name = type;
    }
    this.emit("error", err);
...
SMTPClient.prototype._authenticateUser = function(){
    ...
    var auth = (this._supportedAuth.shift() || "PLAIN").toUpperCase().trim();
...
SMTPClient.prototype._actionGreeting = function(str){
    if(str.substr(0,3) != "220"){
        this._onError(new Error("Invalid greeting from server - "+str));
        return;
    }
...
SMTPClient.prototype._actionHELO = function(str){
    if(str.charAt(0) != "2"){
        this._onError(new Error("Invalid response for EHLO/HELO - "+str));
        return;
    }
    this._authenticateUser.call(this);
};
...
SMTPClient.prototype._actionSTARTTLS = function(str){
    ...
    this._upgradeConnection((function(err, secured){
        if(err){
            this._onError(new Error("Error initiating TLS - "+(err.message || err)));
            return;
        }
        if(this.options.debug){
...
SMTPClient.prototype._actionAUTH_LOGIN_USER = function(str){
    if(str != "334 VXNlcm5hbWU6"){
        this._onError(new Error("Invalid login sequence while waiting for '334 VXNlcm5hbWU6' - "+str));
        return;
    }
...
SMTPClient.prototype._actionAUTH_LOGIN_PASS = function(str){
    if(str != "334 UGFzc3dvcmQ6"){
        this._onError(new Error("Invalid login sequence while waiting for '334 UGFzc3dvcmQ6' - "+str));
        return;
    }
    this._currentAction = this._actionAUTHComplete;
...
SMTPClient.prototype._actionAUTHComplete = function(str){
    if(str.charAt(0) != "2"){
        this._onError(new Error("Invalid login - "+str), "AuthError");
        return;
    }
...
SMTPClient.prototype._actionMAIL = function(str){
    if(Number(str.charAt(0)) != "2"){
        this._onError(new Error("Mail from command failed - " + str), "SenderError");
        return;
    }
...
SMTPClient.prototype._actionDATA = function(str){
    if(Number(str.charAt(0)) != "3"){
        this._onError(new Error("Data command failed - " + str));
        return;
    }

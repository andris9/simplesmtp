...
SMTPClient.prototype.useEnvelope = function(envelope){
    ...
    if(typeof this._envelope.from === "object") {
        this._envelope.from = this._envelope.from.name +" <"+ this._envelope.from.address +">";
    }

    this._envelope.to.forEach(function(to, i) {
        if(typeof this._envelope.from === "object") {
            this._envelope.to[i] = to.name +" <"+ to.address +">";
        }
    }.bind(this));

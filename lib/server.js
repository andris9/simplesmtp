            ...
            if(!self.server.options.disableDNSValidation) {
              dnslib.resolveMx(domain, function(err, addresses){
                  if(err || !addresses || !addresses.length){
                      ...
                      self.client.send("450 4.1.8 <"+email+">: Sender address rejected: Domain not found");
                  }
                  next();
              })
            } else {
        ...
        function (next) {
            if(self.server.options.validateSender){
                self.server.emit("validateSender", self.envelope, email, function(err){
                    if(err){
                        self.client.send("550 5.1.1 <"+email+">: Sender address rejected: User unknown in local sender table");
                    }
                    next();
                });
            } else {
            ...
            if(!self.server.options.disableDNSValidation) {
                dnslib.resolveMx(domain, function(err, addresses){
                    if(err || !addresses || !addresses.length){
                        ...
                        self.client.send("450 4.1.8 <"+email+">: Recipient address rejected: Domain not found");
                    }
                    next();
                });
            } else {
        ...
        function (next) {
            if(self.server.options.validateRecipients){
                self.server.emit("validateRecipient", self.envelope, email, function(err){
                    if(err){
                        self.client.send("550 5.1.1 <"+email+">: Recipient address rejected: User unknown in local recipient table");
                    }
                    next();
                });
            } else {
